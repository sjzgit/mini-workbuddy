...
当前已交付：**第一阶段（工程初始化）**、**第二阶段（模型管理）**、**第三阶段（工具管理）** 及聊天 / Agent / 运行记录 / 评测等阶段——"工具管理"提供内置工具（当前时间 / Shell 命令 / 文件读写 / **询问用户**）的启停管理与统一执行入口：Shell 在执行层拦截危险命令、文件操作限制在授权目录内、**询问用户（ask_user）可在对话中向使用者发起开放式或选项式提问并等待回答**（超时上限 `ASK_USER_TIMEOUT_SECONDS`，默认 300 秒）。
